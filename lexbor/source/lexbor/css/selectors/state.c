...
#define lxb_css_selectors_state_string_dup_m(selectors, name) \
  do {                                                        \
    (status) = lxb_css_syntax_token_string_dup(               \
        lxb_css_syntax_token_string(token),                   \
        (name),                                               \
        (parser)->memory->mraw);                              \
...
lxb_css_selectors_state_complex_list_end(lxb_css_parser_t* parser,
                                         const lxb_css_syntax_token_t* token,
                                         void* ctx) {
  return lxb_css_selectors_state_list_end(parser, token, lxb_css_selectors_state_complex_wo_root);
}
...
lxb_css_selectors_state_relative_list_end(lxb_css_parser_t* parser,
                                          const lxb_css_syntax_token_t* token,
                                          void* ctx) {
  return lxb_css_selectors_state_list_end(parser, token, lxb_css_selectors_state_relative_list_wo_root);
}
...
lxb_css_selectors_state_relative_list_wo_root(lxb_css_parser_t* parser,
                                              const lxb_css_syntax_token_t* token,
                                              void* ctx) {
  return lxb_css_selectors_state_relative_handler(parser, token, ctx, true, false);
}
...
lxb_css_selectors_state_relative_wo_root(lxb_css_parser_t* parser,
                                         const lxb_css_syntax_token_t* token,
                                         void* ctx) {
  return lxb_css_selectors_state_relative_handler(parser, token, ctx, false, false);
}
...
lxb_css_selectors_state_relative_handler(lxb_css_parser_t* parser,
  ...
  case LXB_CSS_SYNTAX_TOKEN_DELIM:
    ...
    case '|':
      ...
      if (token->type == LXB_CSS_SYNTAX_TOKEN_DELIM && lxb_css_syntax_token_delim_char(token) == '|') {
        lxb_css_syntax_parser_consume(parser);
...
lxb_css_selectors_state_compound_list_end(lxb_css_parser_t* parser,
                                          const lxb_css_syntax_token_t* token,
                                          void* ctx) {
  return lxb_css_selectors_state_list_end(parser, token, lxb_css_selectors_state_compound_wo_root);
}
...
again:
  ...
  case LXB_CSS_SYNTAX_TOKEN_COLON:
    ...
    if (token->type == LXB_CSS_SYNTAX_TOKEN_IDENT) {
      ...
      status = lxb_css_selectors_state_pseudo_element_function(parser, token, lxb_css_selectors_state_compound_pseudo);
      break;
    } else if (token->type != LXB_CSS_SYNTAX_TOKEN_FUNCTION) {
      goto unexpected;
    }

    status = lxb_css_selectors_state_pseudo_class_function(parser, token, lxb_css_selectors_state_compound_sub);
    break;
  ...
  case LXB_CSS_SYNTAX_TOKEN__END:
    ...
    if (selectors->combinator > LXB_CSS_SELECTOR_COMBINATOR_CLOSE || selectors->list_last->first == NULL) {
      goto unexpected;
    }
...
lxb_css_selectors_state_compound_pseudo(lxb_css_parser_t* parser,
  ...
  if (token->type == LXB_CSS_SYNTAX_TOKEN_IDENT) {
    ...
    status = lxb_css_selectors_state_pseudo_class_function(parser, token, lxb_css_selectors_state_compound_pseudo);
  }
...
bool lxb_css_selectors_state_simple_list(lxb_css_parser_t* parser,
  ...
  states = lxb_css_parser_states_next(parser, lxb_css_selectors_state_simple_wo_root, lxb_css_selectors_state_simple_list_end, ctx, true);
  if (states == NULL) {
    return lxb_css_parser_memory_fail(parser);
  }
...
lxb_css_selectors_state_simple_list_end(lxb_css_parser_t* parser,
                                        const lxb_css_syntax_token_t* token,
                                        void* ctx) {
  return lxb_css_selectors_state_list_end(parser, token, lxb_css_selectors_state_simple_wo_root);
}
...
again:
  ...
  case LXB_CSS_SYNTAX_TOKEN_COLON:
    ...
    status = lxb_css_selectors_state_pseudo_class_function(parser, token, lxb_css_selectors_state_simple_back);
    break;
...
string_or_ident:
  ...
  if (token->type != LXB_CSS_SYNTAX_TOKEN_STRING && token->type != LXB_CSS_SYNTAX_TOKEN_IDENT) {
    goto failed;
  }
...
lxb_css_selectors_state_ns_ident(lxb_css_parser_t* parser,
  ...
  if (token->type == LXB_CSS_SYNTAX_TOKEN_IDENT) {
    ...
    return status;
  } else if (token->type == LXB_CSS_SYNTAX_TOKEN_DELIM && lxb_css_syntax_token_delim_char(token) == '*') {
    lxb_css_syntax_parser_consume(parser);
...
lxb_css_selectors_state_pseudo_class_function(lxb_css_parser_t* parser,
  ...
  rule = lxb_css_syntax_consume_function(parser, token, &func->cb, success, selectors->list_last);
  if (rule == NULL) {
    goto failed;
  }
...
lxb_css_selectors_state_function_end(lxb_css_parser_t* parser,
  ...
  if (token->type == LXB_CSS_SYNTAX_TOKEN__EOF) {
    (void)lxb_css_log_format(parser->log, LXB_CSS_LOG_SYNTAX_ERROR, "%s. End Of File in pseudo function", lxb_css_selectors_module_name);
  }
...
empty:
  ...
  (void)lxb_css_log_format(parser->log, LXB_CSS_LOG_SYNTAX_ERROR, "%s. Pseudo function can't be empty: %S()", lxb_css_selectors_module_name, &selector->name);
...
lxb_css_selectors_state_function_forgiving(lxb_css_parser_t* parser,
                                           const lxb_css_syntax_token_t* token,
                                           void* ctx, bool failed) {
  return lxb_css_selectors_state_forgiving_cb(parser, token, ctx, lxb_css_selectors_state_complex_list, failed);
}
...
lxb_css_selectors_state_function_forgiving_relative(lxb_css_parser_t* parser,
                                                    const lxb_css_syntax_token_t* token,
                                                    void* ctx, bool failed) {
  return lxb_css_selectors_state_forgiving_cb(parser, token, ctx, lxb_css_selectors_state_relative_list, failed);
}
...
lxb_css_selectors_state_forgiving_cb(lxb_css_parser_t* parser,
  ...
  if (selectors->parent->u.pseudo.type == LXB_CSS_SELECTOR_PSEUDO_CLASS_FUNCTION_WHERE) {
...
lxb_css_selectors_state_function_error(lxb_css_parser_t* parser,
  ...
  if (!selectors->err_in_function) {
    ...
    if (origin != NULL) {
      if (lxb_css_syntax_token_error(parser, origin, "Selectors") == NULL) {
        return NULL;
      }
    }
  }
  ...
  if (cy) {
    ...
    while (token != NULL && token->type != LXB_CSS_SYNTAX_TOKEN__END) {
      if (comma == true && token->type == LXB_CSS_SYNTAX_TOKEN_COMMA && lxb_css_parser_rule_deep(parser) == 0) {
        break;
      }
