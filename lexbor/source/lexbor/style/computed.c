...
static bool
lxb_style_computed_value_equal(const lxb_style_computed_value_t* left,
                               const lxb_style_computed_value_t* right);

static bool
lxb_style_computed_border_equal(const lxb_style_computed_border_t* left,
                                const lxb_style_computed_border_t* right);

static bool
lxb_style_color_equal(const lxb_style_color_t* left,
                      const lxb_style_color_t* right);
...
void lxb_style_computed_destroy(lxb_style_computed_t* style) {
  if (style == NULL) {
    return;
  }
...
void lxb_style_computed_ref(lxb_style_computed_t* style) {
  if (style != NULL && style->refs != SIZE_MAX) {
    style->refs++;
  }
}

void lxb_style_computed_unref(lxb_style_computed_t* style) {
  if (style == NULL) {
    return;
  }
...
bool lxb_style_computed_equal(const lxb_style_computed_t* left,
  ...
  return lxb_style_computed_inherited_equal(left->inherited, right->inherited) && lxb_style_computed_non_inherited_equal(left->non_inherited, right->non_inherited) && lxb_style_computed_box_equal(left->box, right->box) && lxb_style_computed_surround_equal(left->surround, right->surround) && lxb_style_computed_visual_equal(left->visual, right->visual);
}
...
lxb_style_computed_set_initial(lxb_style_computed_t* style,
  ...
  if (inherited == NULL || rare == NULL || non_inherited == NULL || box == NULL || surround == NULL || visual == NULL) {
    /* inherited->rare is assigned only after this allocation gate. */
...
lxb_style_computed_rare_inherited_equal(
  ...
  return left->text_align == right->text_align &&
...
lxb_style_computed_inherited_equal(
  ...
  return lxb_style_color_equal(&left->color, &right->color) &&
         left->direction == right->direction &&
         left->visibility == right->visibility &&
         left->font_size == right->font_size &&
         left->line_height == right->line_height &&
         left->font_weight_type == right->font_weight_type &&
         left->font_weight == right->font_weight &&
         left->font_style == right->font_style &&
         left->font_stretch_type == right->font_stretch_type &&
         left->font_stretch == right->font_stretch &&
...
lxb_style_computed_non_inherited_equal(
  ...
  return left->display_a == right->display_a &&
         ...
         left->z_index_auto == right->z_index_auto &&
...
lxb_style_computed_box_equal(const lxb_style_computed_box_t* left,
  ...
  if (!lxb_style_computed_value_equal(&left->width, &right->width) ||
      !lxb_style_computed_value_equal(&left->height, &right->height) ||
      !lxb_style_computed_value_equal(&left->min_width, &right->min_width) ||
      !lxb_style_computed_value_equal(&left->min_height, &right->min_height) ||
      !lxb_style_computed_value_equal(&left->max_width, &right->max_width) ||
      !lxb_style_computed_value_equal(&left->max_height, &right->max_height)) {
    return false;
  }

  for (i = 0; i < LXB_STYLE_EDGE__LAST_ENTRY; i++) {
    if (!lxb_style_computed_value_equal(&left->inset[i], &right->inset[i])) {
      return false;
    }
  }
...
lxb_style_computed_surround_equal(const lxb_style_computed_surround_t* left,
  ...
  for (i = 0; i < LXB_STYLE_EDGE__LAST_ENTRY; i++) {
    if (!lxb_style_computed_value_equal(&left->margin[i], &right->margin[i]) ||
        !lxb_style_computed_value_equal(&left->padding[i], &right->padding[i]) ||
        !lxb_style_computed_border_equal(&left->border[i], &right->border[i])) {
      return false;
    }
  }
...
lxb_style_computed_visual_equal(const lxb_style_computed_visual_t* left,
  ...
  return lxb_style_color_equal(&left->background_color, &right->background_color);
}
...
lxb_style_computed_value_equal(const lxb_style_computed_value_t* left,
  ...
  case LXB_STYLE_COMPUTED_VALUE_NONE:
  default:
    return true;
  }
}
...
lxb_style_computed_border_equal(const lxb_style_computed_border_t* left,
  ...
  return lxb_style_computed_value_equal(&left->width, &right->width) &&
         left->style == right->style &&
         lxb_style_color_equal(&left->color, &right->color);
}
