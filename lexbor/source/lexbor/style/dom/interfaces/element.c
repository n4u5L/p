...
static void
lxb_dom_element_computed_style_invalidate_subtree(lxb_dom_element_t* element);

static lexbor_action_t
lxb_dom_element_computed_style_dirty_cb(lxb_dom_node_t* node, void* ctx);
...
lxb_dom_element_computed_style_invalidate_subtree(lxb_dom_element_t* element) {
  ...
  lxb_dom_node_simple_walk(lxb_dom_interface_node(element),
                           lxb_dom_element_computed_style_dirty_cb, NULL);
}
...
lxb_dom_element_style_attach(lxb_dom_element_t* element,
  ...
  return lxb_selectors_find(css->selectors, lxb_dom_interface_node(element), style->selector, lxb_style_document_cb, style);
}
...
lxb_style_document_cb(lxb_dom_node_t* node,
  ...
  lxb_dom_element_computed_style_invalidate_subtree(
...
lxb_dom_element_style_append(lxb_dom_element_t* element,
  ...
  lxb_dom_element_computed_style_invalidate_subtree(element);
  ...
  if (node != NULL) {
    ...
    status = lxb_dom_element_style_weak_append(doc, node, node->entry.value, node->sp);
    if (status != LXB_STATUS_OK) {
      return status;
    }
...
lxb_dom_element_style_walk(lxb_dom_element_t* element,
  ...
  return lexbor_avl_foreach(NULL, &element->style, lxb_dom_element_style_walk_cb, &walk);
}
...
lxb_dom_element_style_walk_cb(lexbor_avl_t* avl, lexbor_avl_node_t** root,
  ...
  while (weak != NULL) {
    status = walk->cb(walk->element, weak->value, walk->ctx, weak->sp, true);
    if (status != LXB_STATUS_OK) {
      return status;
    }
...
lxb_dom_element_style_parse(lxb_dom_element_t* element,
  ...
  return lxb_dom_element_style_list_append(element, list, lxb_css_selector_sp_up_s(0));
}

void lxb_dom_element_style_remove_by_name(lxb_dom_element_t* element,
                                          const lxb_char_t* name, size_t size) {
  uintptr_t id;
  lxb_dom_document_t* doc = lxb_dom_element_document(element);
...
void lxb_dom_element_style_remove_by_id(lxb_dom_element_t* element, uintptr_t id) {
  lxb_style_node_t* node;
  lxb_dom_document_t* doc = lxb_dom_element_document(element);
...
lxb_dom_element_style_remove_all_not(lxb_dom_element_t* element,
  ...
  if (style->weak == NULL) {
    lexbor_avl_remove_by_node(doc->css->styles, root, (lexbor_avl_node_t*)style);
    return NULL;
  }
...
lxb_dom_element_style_remove_non_inline(lxb_dom_element_t* element) {
  ...
  return lexbor_avl_foreach(doc->css->styles, &element->style, lxb_dom_element_style_remove_dirty_cb, element);
}
...
lxb_dom_element_style_remove_by_list(lxb_dom_element_t* element,
  ...
  while (weak != NULL) {
    ...
    if (condition & LXB_DOM_ELEMENT_CONDITION_DIRTY_STYLE) {
      ...
      if (((lxb_css_rule_declaration_t*)weak->value)->rule.parent == (lxb_css_rule_t*)list) {
        lexbor_dobject_free(doc->css->weak, weak);
  ...
  if (!(condition & LXB_DOM_ELEMENT_CONDITION_DIRTY_STYLE) || lxb_css_selector_sp_s(style->sp)) {
    if (((lxb_css_rule_declaration_t*)style->entry.value)->rule.parent != (lxb_css_rule_t*)list) {
      return style;
    }
  }
...
lxb_dom_element_style_serialize(lxb_dom_element_t* element,
  ...
  return lexbor_avl_foreach(NULL, &element->style, lxb_dom_element_style_serialize_cb, &context);
}
...
lxb_dom_element_style_serialize_cb(lexbor_avl_t* avl, lexbor_avl_node_t** root,
  ...
  if (element->condition & LXB_DOM_ELEMENT_CONDITION_DIRTY_STYLE && !lxb_css_selector_sp_s(style->sp)) {
    return LXB_STATUS_OK;
  }

  if (!context->is_first) {
    lexbor_serialize_write(context->cb, splt.data, splt.length, context->ctx, status);
  }
...
lxb_dom_element_style_serialize_str(lxb_dom_element_t* element, lexbor_str_t* str,
  ...
  return lxb_dom_element_style_serialize(element, opt, lxb_dom_element_style_serialize_str_cb, &context);
}
