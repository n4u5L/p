...
static bool
layout_object_is_anonymous_block(layout_object_t *object)
{
    return object != NULL
...
static layout_tree_node_t *
layout_tree_create_anonymous_block(layout_tree_t *tree,
...
static layout_tree_node_t *
layout_tree_add_child_to_block(layout_tree_t *tree, layout_tree_node_t *parent,
                               ...
                               const lxb_style_computed_t *child_style)
{
    ...
    if (layout_object_is_anonymous_block(last_child)) {
        anonymous = layout_tree_node_for_object(tree, last_child);
    }
    else {
        anonymous = layout_tree_create_anonymous_block(tree, parent,
                                                       child_style);
    }
                                             ...
                                             uint32_t *out_index)
{
    ...
    *out_index = 0;
    ...
    if (result->fragment_index_counters_length
        == result->fragment_index_counters_capacity) {
        capacity = result->fragment_index_counters_capacity == 0
            ? 8 : result->fragment_index_counters_capacity * 2;
        ...
        if (result->fragment_index_counters == NULL) {
            ...
            counters = lexbor_mraw_realloc(
                result->arena, result->fragment_index_counters,
                capacity * sizeof(layout_fragment_index_counter_t));
        }
    ...
    result->fragment_index_counters[
        result->fragment_index_counters_length].object = object;
    result->fragment_index_counters[
        result->fragment_index_counters_length].next_index = 1;
...
layout_dom_object_id(layout_t *layout, lxb_dom_node_t *node)
{
    return layout_mix_identity_uint64(
        ...
        ^ (layout == NULL ? 0 : layout->identity_salt));
}
...
layout_anonymous_object_id(layout_t *layout, layout_object_t *object)
{
    return layout_mix_identity_uint64(
        ...
        ^ (layout == NULL ? 0 : layout->anonymous_identity_salt)
        ^ (layout == NULL ? 0 : layout->identity_salt));
}
...
layout_result_identity_generation(layout_t *layout, layout_result_t *result)
{
    return layout_mix_identity_uint64(
        ...
        ^ (layout == NULL ? 0 : layout->generation_salt)
        ^ (layout == NULL ? 0 : layout->identity_salt));
}
...
static bool
layout_style_display_none(const lxb_style_computed_t *style)
{
    const lxb_style_computed_non_inherited_t *non_inherited;
...
static bool
layout_style_display_contents(const lxb_style_computed_t *style)
{
    const lxb_style_computed_non_inherited_t *non_inherited;
...
static bool
layout_style_is_block_capable(const lxb_style_computed_t *style)
{
    const lxb_style_computed_non_inherited_t *non_inherited;
...
layout_object_update_style_derived_bits(layout_object_t *object)
{
    ...
    if ((object->bitfields
         & LAYOUT_OBJECT_INTERNAL_VIEWPORT_CONTAINING_BLOCK) != 0) {
    ...
    layout_object_set_can_contain_absolute_position(
        object, position != LXB_CSS_POSITION_STATIC);
}
...
{
    return object != NULL
        && (object->bitfields & LAYOUT_OBJECT_POINTER_EVENTS_NONE) == 0
        && layout_object_visible(object)
        && !layout_object_display_none(object);
}
...
{
    if (object == NULL || object->style == NULL
        || object->style->non_inherited == NULL) {
...
{
    return layout_object_is_absolute_positioned(object)
        || layout_object_is_fixed_positioned(object);
}
...
layout_object_mark_spanner_or_out_of_flow_parent_change(layout_object_t *object)
{
    ...
    if (object == NULL
        || (!layout_object_is_out_of_flow_positioned(object)
            && !layout_object_is_column_spanner(object))) {
        return;
    }
    ...
    for (layout_object_t *ancestor = object->parent; ancestor != NULL
         && ancestor != containing_block; ancestor = ancestor->parent) {
        ancestor->dirty_bits |= LAYOUT_DIRTY_CHILD_FULL;
    }
...
layout_rect_contains(layout_rect_t rect, layout_point_t point,
                     layout_overflow_clip_t clip)
{
    if ((clip & LAYOUT_OVERFLOW_CLIP_X) != 0
        && (point.x < 0.0 || point.x > rect.width)) {
        return false;
    }

    if ((clip & LAYOUT_OVERFLOW_CLIP_Y) != 0
        && (point.y < 0.0 || point.y > rect.height)) {
        return false;
    }
...
layout_fragment_content_rect_unsafe(layout_fragment_t *fragment)
{
    ...
    horizontal_edges = rect.x + fragment->padding.right
        + fragment->border.right;
    vertical_edges = rect.y + fragment->padding.bottom
...
layout_fragment_self_contains(layout_fragment_t *fragment,
                               layout_point_t local_point)
{
    return local_point.x >= 0.0 && local_point.y >= 0.0
        && local_point.x <= fragment->size.width
        && local_point.y <= fragment->size.height;
}
                              ...
                              bool has_external_children)
{
     ...
     */
    if ((fragment->size.width <= 0.0 || fragment->size.height <= 0.0)
        && fragment->children_length == 0 && !has_external_children) {
        return false;
    }
...
{
    if (fragment == NULL
        || (coordinate_flags & LAYOUT_COORDINATE_IGNORE_TRANSFORMS) != 0) {
        return local_point;
    }
                          ...
                          size_t capacity)
{
    ...
    for (size_t i = 0; i < current->children_length; i++) {
        ...
        if (layout_fragment_find_path(current->children[i].fragment, target,
                                      ...
                                      layout_oof_positioned_t *oof)
{
    ...
    fragment_index = 0;
    return layout_result_oof_fragment_at_object(result, oof->containing_block,
                                                &fragment_index);
}
                                     ...
                                     size_t *fragment_index)
{
    ...
    for (size_t i = 0; i < result->oofs_length; i++) {
        if (result->oofs[i].fragment != NULL
            && result->oofs[i].fragment->object == target_object) {
            if (*fragment_index == 0) {
...
layout_result_out_of_flow_needs_layout(layout_result_t *result)
{
    ...
    for (size_t i = 0; i < result->oofs_length; i++) {
        if (layout_object_subtree_needs_layout(result->oofs[i].object)
            || layout_object_subtree_needs_layout(
                result->oofs[i].containing_block)
            || layout_fragment_tree_needs_layout(result->oofs[i].fragment)) {
            return true;
        }
    }
                                  ...
                                  layout_fragment_t *target)
{
    ...
    if (root == NULL || target == NULL
        || !layout_fragment_find_path(root, target, links, &length,
                                     ...
                                     int fallback_stacking_order)
{
    ...
    if (has_z_index
        && layout_object_position(object) != LXB_CSS_POSITION_STATIC) {
        return z_index;
    }
                                                 ...
                                                 layout_point_t *out_root)
{
    ...
    if (oof != NULL) {
        return layout_result_oof_local_point_to_root(
            result, oof, local_point, LAYOUT_COORDINATE_APPLY_TRANSFORMS,
            out_root);
    }
    ...
    return layout_fragment_local_point_to_root(
        result->root_fragment, fragment, local_point,
        LAYOUT_COORDINATE_APPLY_TRANSFORMS, out_root);
}
                                           ...
                                           layout_point_t *out_local)
{
    ...
    if (oof != NULL) {
        return layout_result_root_point_to_oof_fragment(
            result, oof, root_point, LAYOUT_COORDINATE_APPLY_TRANSFORMS,
            out_local);
    }
    ...
    return layout_fragment_point_from_root(
        result->root_fragment, fragment, root_point,
        LAYOUT_COORDINATE_APPLY_TRANSFORMS, out_local);
}
                                        ...
                                        layout_object_t *object)
{
    ...
    fragment = layout_result_fragment_at_object(result->root_fragment, object,
                                                &fragment_index);
    if (fragment != NULL) {
        return fragment;
    }
    ...
    fragment_index = 0;
    return layout_result_oof_fragment_at_object(result, object,
                                                &fragment_index);
}
                                           ...
                                           layout_point_t *out_offset)
{
    ...
    if (result == NULL || oof == NULL || paint_parent == NULL
        || out_offset == NULL) {
        return LXB_STATUS_ERROR_OBJECT_IS_NULL;
    }
    ...
    status = layout_visual_order_fragment_local_point_to_root(
        result, containing_fragment, oof->offset, &root_point);
    if (status != LXB_STATUS_OK) {
        return status;
    }

    return layout_visual_order_root_point_to_fragment(result, paint_parent,
                                                      root_point, out_offset);
}
...
layout_visual_order_item_phase(const layout_visual_order_item_t *item)
{
    ...
    if (item->is_oof || item->placement == LAYOUT_FRAGMENT_PLACEMENT_OUT_OF_FLOW
        || (object != NULL
            && layout_object_position(object) != LXB_CSS_POSITION_STATIC)
        || layout_fragment_establishes_stacking_context(item->fragment)) {
        return 3u;
    }
                                      ...
                                      layout_fragment_t *fragment)
{
    ...
    for (size_t i = 0; i < result->oofs_length; i++) {
        if (layout_visual_order_oof_parent_fragment(result, &result->oofs[i])
            == fragment) {
            count++;
        }
    }
                                  ...
                                  size_t *out_length)
{
    ...
    for (size_t i = 0; i < result->oofs_length && index < capacity; i++) {
        ...
        status = layout_visual_order_oof_offset_from_parent(
            result, oof, paint_parent, &paint_offset);
        if (status != LXB_STATUS_OK) {
            return status;
        }
                                  ...
                                  layout_point_t local_point)
{
    ...
    if (item->is_oof) {
        return layout_hit_test_oof_fragment(result, item->oof, local_point,
                                            item->offset);
    }
                               ...
                               layout_point_t local_point)
{
    ...
    if (layout_fragment_hidden_for_paint(fragment)
        || !layout_object_can_hit_test_descendants(fragment->object)) {
        return NULL;
    }

    if (layout_object_hit_clip_axes(fragment->object, &clip)
        && !layout_rect_contains(layout_fragment_local_rect_unsafe(fragment),
                                 local_point, clip)) {
        return NULL;
    }
    ...
    if (!layout_fragment_may_intersect(fragment, local_point,
                                       oof_items != 0)) {
        return NULL;
    }
    ...
    if (items_length != 0) {
        ...
        status = layout_visual_order_collect_items(
            result, fragment, items, items_length, &items_length);
        if (status != LXB_STATUS_OK) {
            lexbor_free(items);
            return NULL;
        }
    ...
    if (layout_object_hit_testable(fragment->object)
        && !layout_fragment_opaque(fragment)
        && layout_fragment_self_contains(fragment, local_point)) {
        return fragment;
    }
                         ...
                         layout_point_t point_in_parent)
{
    ...
    if (!layout_fragment_ancestor_to_local(fragment, point_in_parent,
                             ...
                             layout_point_t paint_offset)
{
    ...
    if (!layout_fragment_ancestor_to_local(oof->fragment, point,
                                           LAYOUT_COORDINATE_APPLY_TRANSFORMS,
                                           &point)) {
        return NULL;
    }
...
layout_init(layout_t *layout)
{
    ...
    status = lexbor_dobject_init(layout->objects, LAYOUT_OBJECT_CHUNK,
                                 sizeof(layout_object_t));
    if (status != LXB_STATUS_OK) {
        layout_destroy(layout, false);
        return status;
    }

    status = lexbor_dobject_init(layout->blocks, LAYOUT_BLOCK_CHUNK,
                                 sizeof(layout_block_t));
    if (status != LXB_STATUS_OK) {
        layout_destroy(layout, false);
        return status;
    }
...
layout_clean(layout_t *layout)
{
    ...
    layout->anonymous_identity_salt = layout_mix_identity_uint64(
        layout->anonymous_identity_salt ^ (uint64_t) (uintptr_t) layout
        ^ LAYOUT_ANONYMOUS_IDENTITY_SALT);
}
                       ...
                       layout_tree_node_t *parent)
{
    ...
    if (style != NULL && !layout_style_display_contents(style)) {
        ...
        object = layout_object_create_for_node(tree->layout, dom_node);
        if (object == NULL) {
            return LXB_STATUS_ERROR_MEMORY_ALLOCATION;
        }
        ...
        if (parent == NULL) {
            ...
            layout_parent = layout_tree_add_child_to_block(tree, parent, node,
                                                           style);
    ...
    for (lxb_dom_node_t *child = dom_node->first_child; child != NULL;
         child = child->next) {
        lxb_status_t status = layout_tree_build_node(tree, child,
                                                     current_parent);
        if (status != LXB_STATUS_OK) {
            return status;
        }
...
void
layout_tree_clean(layout_tree_t *tree)
{
    if (tree == NULL) {
        return;
    }
...
layout_tree_build(layout_tree_t *tree, lxb_dom_node_t *root_node)
{
    ...
    status = layout_tree_build_node(tree, root_node, NULL);
    if (status != LXB_STATUS_OK) {
        layout_tree_clean(tree);
    }
                                        ...
                                        layout_object_t *stay_within)
{
    ...
    if (stay_within != NULL
        && !layout_tree_object_is_recorded(tree, stay_within)) {
        return false;
    }
                                    ...
                                    layout_object_t *stay_within)
{
    ...
    if (!layout_tree_object_traversal_args_valid(tree, object,
                                                 stay_within)) {
        return NULL;
    }
                                        ...
                                        layout_object_t *stay_within)
{
    ...
    while ((last_child = layout_tree_object_last_child(tree, previous))
           != NULL) {
        previous = last_child;
    }
...
layout_tree_object_previous_in_postorder_before_children(
    layout_tree_t *tree, layout_object_t *object, layout_object_t *stay_within)
{
    ...
    for (layout_object_t *current = object; current != NULL;
        ...
        if (previous != NULL) {
            return layout_tree_object_is_recorded(tree, previous)
                ? previous : NULL;
        }
...
layout_result_init(layout_result_t *result, layout_t *layout)
{
    ...
    status = lexbor_dobject_init(result->fragments, LAYOUT_FRAGMENT_CHUNK,
                                 sizeof(layout_fragment_t));
    if (status != LXB_STATUS_OK) {
        layout_result_destroy(result, false);
        return status;
    }
...
void
layout_result_clean(layout_result_t *result)
{
    if (result == NULL) {
        return;
    }
...
bool
layout_result_is_frozen(layout_result_t *result)
{
                                         ...
                                         bool fixed_position)
{
    return layout_result_add_out_of_flow_positioned_in_fragment(
        result, object, containing_block, NULL, fragment, offset,
        fixed_position);
}
    ...
    bool fixed_position)
{
    ...
    if (result == NULL || object == NULL || containing_block == NULL
        || fragment == NULL) {
        return LXB_STATUS_ERROR_OBJECT_IS_NULL;
    }
    ...
    if (containing_fragment != NULL
        && (containing_fragment->result != result
            || containing_fragment->object != containing_block)) {
        return LXB_STATUS_ERROR_WRONG_ARGS;
    }

    if (fixed_position) {
        if (layout_object_containing_block_for_fixed(object)
            != containing_block) {
            return LXB_STATUS_ERROR_WRONG_ARGS;
        }
    }
    else if (layout_object_containing_block_for_absolute(object)
             != containing_block) {
        return LXB_STATUS_ERROR_WRONG_ARGS;
    }
    ...
    if (result->oofs_length == result->oofs_capacity) {
        ...
        if (result->oofs == NULL) {
            oofs = lexbor_mraw_calloc(result->arena,
                                      capacity
                                          * sizeof(layout_oof_positioned_t));
        }
        else {
            oofs = lexbor_mraw_realloc(result->arena, result->oofs,
                                       capacity
                                           * sizeof(layout_oof_positioned_t));
        }
...
int
layout_oof_positioned_stacking_order(layout_oof_positioned_t *oof)
{
    return oof == NULL ? 0 : oof->stacking_order;
}

bool
layout_oof_positioned_is_fixed(layout_oof_positioned_t *oof)
{
...
layout_object_t *
layout_object_create_for_node(layout_t *layout, lxb_dom_node_t *node)
{
    layout_object_t *object;
                               ...
                               const lxb_style_computed_t *style)
{
    ...
    object->bitfields = LAYOUT_OBJECT_ANONYMOUS
...
void
layout_object_child_list_init(layout_object_child_list_t *children)
{
    if (children == NULL) {
        return;
    }
...
{
    if (object == NULL
        || (object->bitfields & LAYOUT_OBJECT_ANONYMOUS) != 0) {
        return NULL;
    }
...
bool
layout_object_is_anonymous(layout_object_t *object)
{
    return object != NULL
        && (object->bitfields & LAYOUT_OBJECT_ANONYMOUS) != 0;
}
...
bool
layout_object_can_have_children(layout_object_t *object)
{
    return object != NULL
        && (object->bitfields & LAYOUT_OBJECT_INTERNAL_BLOCK) != 0;
}
...
void
layout_object_set_pointer_events_none(layout_object_t *object, bool value)
{
    layout_object_set_bit(object, LAYOUT_OBJECT_POINTER_EVENTS_NONE, value);
}

bool
layout_object_pointer_events_none(layout_object_t *object)
{
    return object != NULL
        && (object->bitfields & LAYOUT_OBJECT_POINTER_EVENTS_NONE) != 0;
}
...
void
layout_object_set_has_clip_path(layout_object_t *object, bool value)
{
    layout_object_set_bit(object, LAYOUT_OBJECT_HAS_CLIP_PATH, value);
}

bool
layout_object_has_clip_path(layout_object_t *object)
{
    return object != NULL
        && (object->bitfields & LAYOUT_OBJECT_HAS_CLIP_PATH) != 0;
}

void
layout_object_set_can_contain_absolute_position(layout_object_t *object,
                                                bool value)
{
...
bool
layout_object_can_contain_absolute_position(layout_object_t *object)
{
    return object != NULL
        && (object->bitfields
            & LAYOUT_OBJECT_CAN_CONTAIN_ABSOLUTE_POSITION) != 0;
}

void
layout_object_set_can_contain_fixed_position(layout_object_t *object,
                                             bool value)
{
    layout_object_set_bit(object, LAYOUT_OBJECT_CAN_CONTAIN_FIXED_POSITION,
                          value);
}

bool
layout_object_can_contain_fixed_position(layout_object_t *object)
{
    return object != NULL
        && (object->bitfields & LAYOUT_OBJECT_CAN_CONTAIN_FIXED_POSITION)
               != 0;
}

void
layout_object_set_column_spanner_container(layout_object_t *object,
                                           bool value)
{
    layout_object_set_bit(object, LAYOUT_OBJECT_COLUMN_SPANNER_CONTAINER,
                          value);
}

bool
layout_object_is_column_spanner_container(layout_object_t *object)
{
    return object != NULL
        && (object->bitfields & LAYOUT_OBJECT_COLUMN_SPANNER_CONTAINER)
               != 0;
}

void
layout_object_set_column_spanner(layout_object_t *object, bool value)
{
    layout_object_set_bit(object, LAYOUT_OBJECT_COLUMN_SPANNER, value);
}

bool
layout_object_is_column_spanner(layout_object_t *object)
{
    return object != NULL
        && (object->bitfields & LAYOUT_OBJECT_COLUMN_SPANNER) != 0;
}

void
layout_object_set_can_traverse_physical_fragments(layout_object_t *object,
                                                  bool value)
{
    layout_object_set_bit(object,
                          LAYOUT_OBJECT_CAN_TRAVERSE_PHYSICAL_FRAGMENTS,
...
bool
layout_object_can_traverse_physical_fragments(layout_object_t *object)
{
    return object != NULL
        && (object->bitfields
            & LAYOUT_OBJECT_CAN_TRAVERSE_PHYSICAL_FRAGMENTS) != 0;
}

void
layout_object_set_native_embed_token(layout_object_t *object,
...
bool
layout_object_is_native_embed_host(layout_object_t *object)
{
...
bool
layout_object_is_before_in_preorder(layout_object_t *object,
                                    layout_object_t *other)
{
    layout_object_common_ancestor_data_t data;
...
layout_object_child_list_append(layout_object_child_list_t *children,
                                ...
                                layout_object_t *child)
{
    return layout_object_child_list_insert_before(children, parent, child,
                                                 NULL);
}
                                       ...
                                       layout_object_t *before_child)
{
    ...
    while (before_child != NULL && before_child->parent != NULL
           && before_child->parent != parent) {
...
bool
layout_fragment_data_is_first(layout_fragment_data_t *fragment_data)
{
    return fragment_data != NULL
        && (fragment_data->flags & LAYOUT_FRAGMENT_DATA_IS_FIRST) != 0;
}

void
layout_fragment_data_set_paint_offset(layout_fragment_data_t *fragment_data,
                                      layout_point_t paint_offset)
{
...
bool
layout_object_can_contain_out_of_flow_positioned(layout_object_t *object,
                                                 bool fixed_position)
{
    if (object == NULL) {
        return false;
...
void
layout_object_set_self_needs_full_layout(layout_object_t *object, bool value)
{
    if (object == NULL) {
        return;
    }
...
void
layout_object_set_child_needs_full_layout(layout_object_t *object, bool value)
{
    if (object == NULL) {
        return;
    }
...
void
layout_object_set_needs_simplified_layout(layout_object_t *object, bool value)
{
    if (object == NULL) {
        return;
    }
...
bool
layout_object_self_needs_full_layout(layout_object_t *object)
{
    return object != NULL
        && (object->dirty_bits & LAYOUT_DIRTY_SELF_FULL) != 0;
}

bool
layout_object_child_needs_full_layout(layout_object_t *object)
{
    return object != NULL
        && (object->dirty_bits & LAYOUT_DIRTY_CHILD_FULL) != 0;
}

bool
layout_object_needs_simplified_layout(layout_object_t *object)
{
    return object != NULL
        && (object->dirty_bits & LAYOUT_DIRTY_SIMPLIFIED) != 0;
}

bool
layout_object_subtree_needs_layout(layout_object_t *object)
{
    if (object == NULL) {
        return true;
    }

    return (object->dirty_bits
            & (LAYOUT_DIRTY_SELF_FULL | LAYOUT_DIRTY_CHILD_FULL
               | LAYOUT_DIRTY_SIMPLIFIED))
        != 0;
}

void
layout_object_clear_needs_layout(layout_object_t *object)
{
    if (object == NULL) {
        return;
    }
    ...
    object->dirty_bits &= ~(LAYOUT_DIRTY_SELF_FULL
                            | LAYOUT_DIRTY_CHILD_FULL
                            | LAYOUT_DIRTY_SIMPLIFIED);
}

bool
layout_result_needs_layout(layout_result_t *result)
{
    if (result == NULL || result->root_fragment == NULL) {
        return true;
    }

    return layout_fragment_tree_needs_layout(result->root_fragment)
        || layout_result_out_of_flow_needs_layout(result);
}
                                   ...
                                   layout_fragment_t *root_fragment)
{
    ...
    if (root_fragment != NULL) {
        return layout_fragment_tree_needs_layout(root_fragment)
            || layout_result_out_of_flow_needs_layout(result);
    }
                       ...
                       const layout_fragment_init_t *init)
{
    ...
    if (result == NULL || result->fragments == NULL || init == NULL
        || init->object == NULL) {
        return NULL;
    }
    ...
    if (!layout_result_next_fragment_index_for_object(result, init->object,
                                                      &fragment_index)) {
        return NULL;
    }
    ...
    fragment->border_sides = init->border_sides != LAYOUT_BOX_SIDE_NONE
        ? init->border_sides : layout_box_sides_from_edges(init->border);

    if (fragment->transform.a == 0.0 && fragment->transform.b == 0.0
        && fragment->transform.c == 0.0 && fragment->transform.d == 0.0
        && fragment->transform.e == 0.0 && fragment->transform.f == 0.0) {
                                       ...
                                       layout_fragment_t **out_fragment)
{
    ...
    *out_fragment = NULL;
    ...
    if (result->frozen || source_fragment->result == NULL
        || !source_fragment->result->frozen
        || layout_fragment_tree_needs_layout(source_fragment)) {
        return LXB_STATUS_ERROR_WRONG_STAGE;
    }
    ...
    for (size_t i = 0; i < source_fragment->children_length; i++) {
        ...
        status = layout_fragment_append_child_with_placement(
            clone, child_clone, source_link->offset, source_link->placement,
                              ...
                              layout_fragment_t *source_fragment)
{
    ...
    if (layout_fragment_clone_subtree_internal(result, source_fragment,
                                              &clone) != LXB_STATUS_OK) {
        return NULL;
    }
                                    ...
                                    layout_fragment_t **out_cloned_child)
{
    ...
    if (parent == NULL || parent->result == NULL || source_link == NULL
        || source_link->fragment == NULL) {
        return LXB_STATUS_ERROR_WRONG_ARGS;
    }
    ...
    status = layout_fragment_append_child_with_placement(
        parent, clone, offset, source_link->placement,
        source_link->stacking_order);
    if (status != LXB_STATUS_OK) {
        return status;
    }
                             ...
                             layout_point_t offset)
{
    return layout_fragment_append_child_with_stacking_order(
        parent, child, offset, child == NULL ? 0 : child->stacking_order);
}
                                                ...
                                                int stacking_order)
{
    return layout_fragment_append_child_with_placement(
        parent, child, offset, LAYOUT_FRAGMENT_PLACEMENT_NORMAL,
        stacking_order);
}
    ...
    int stacking_order)
{
    ...
    if (parent == NULL || child == NULL || parent->result == NULL
        || parent->result->arena == NULL) {
        return LXB_STATUS_ERROR_WRONG_ARGS;
    }
    ...
    if (child->result != parent->result
        || layout_fragment_tree_contains(parent, child)
        || layout_fragment_tree_contains(child, parent)) {
        return LXB_STATUS_ERROR_WRONG_ARGS;
    }

    if (parent->children_length == parent->children_capacity) {
        capacity = parent->children_capacity == 0
            ? 4 : parent->children_capacity * 2;

        if (parent->children == NULL) {
            children = lexbor_mraw_calloc(parent->result->arena,
                                          capacity
                                              * sizeof(layout_fragment_link_t));
        }
        else {
...
int
layout_fragment_stacking_order(layout_fragment_t *fragment)
{
...
{
    if (!layout_fragment_children_valid(fragment)
        || index >= fragment->children_length) {
        return NULL;
    }
...
int
layout_fragment_link_stacking_order(layout_fragment_link_t *link)
{
...
bool
layout_fragment_hidden_for_paint(layout_fragment_t *fragment)
{
    return fragment != NULL
        && (fragment->flags & LAYOUT_FRAGMENT_HIDDEN_FOR_PAINT) != 0;
}

bool
layout_fragment_opaque(layout_fragment_t *fragment)
{
    return fragment != NULL
        && (fragment->flags & LAYOUT_FRAGMENT_OPAQUE) != 0;
}

bool
layout_fragment_children_valid(layout_fragment_t *fragment)
{
    return fragment != NULL
        && (fragment->flags & LAYOUT_FRAGMENT_CHILDREN_VALID) != 0;
}
...
void
layout_fragment_invalidate_children(layout_fragment_t *fragment)
{
                                  ...
                                  layout_fragment_t **out_fragment)
{
    ...
    *out_fragment = NULL;
    ...
    fragment = layout_result_fragment_at_object(result->root_fragment,
                                               ...
                                               &fragment_index);
    if (fragment == NULL) {
        fragment = layout_result_oof_fragment_at_object(result, target_object,
                                                       &fragment_index);
                              ...
                              unsigned dirty_bits)
{
    ...
    return layout_scene_diff_append(
        diff, op, key, old_parent_key, new_parent_key,
        old_previous_sibling_key, new_previous_sibling_key, old_index,
        new_index, old_parent_index, new_parent_index, old_slot, new_slot,
        old_previous_sibling_index, new_previous_sibling_index,
        old_sequence, new_sequence, old_placement, new_placement,
        old_stacking_order,
        new_stacking_order, dirty_bits);
}
                       ...
                       int stacking_order)
{
    ...
    if (layout_object_is_native_embed_host(object)) {
        ...
        node->layer_hints |= LAYOUT_SCENE_LAYER_HINT_NATIVE_EMBED
            | LAYOUT_SCENE_LAYER_HINT_REPAINT_BOUNDARY;
    }
    ...
    if (!layout_scene_transform_is_identity(node->transform)) {
        node->layer_hints |= LAYOUT_SCENE_LAYER_HINT_TRANSFORM
            | LAYOUT_SCENE_LAYER_HINT_REPAINT_BOUNDARY;
    }
    if (node->clip_axes != LAYOUT_OVERFLOW_CLIP_NONE) {
        node->layer_hints |= LAYOUT_SCENE_LAYER_HINT_CLIP
            | LAYOUT_SCENE_LAYER_HINT_REPAINT_BOUNDARY;
    }
    if (node->opacity < 1.0) {
        node->layer_hints |= LAYOUT_SCENE_LAYER_HINT_OPACITY
            | LAYOUT_SCENE_LAYER_HINT_REPAINT_BOUNDARY;
    }
}
                              ...
                              size_t *out_index)
{
    ...
    layout_scene_node_fill(node, fragment, index, parent_index, offset,
                           placement, sequence, stacking_order);
    plan->length++;
                                       ...
                                       size_t *out_index)
{
    ...
    status = layout_scene_plan_append_node(plan, fragment, parent_index,
                                           offset, placement, sequence,
                                           stacking_order, &index);
    if (status != LXB_STATUS_OK) {
        return status;
    }

    item_count = layout_fragment_child_count(fragment)
        + layout_visual_order_out_of_flow_count(plan->result, fragment);
    if (item_count != 0) {
        ...
        status = layout_visual_order_collect_items(
            plan->result, fragment, items, item_count, &item_count);
        if (status != LXB_STATUS_OK) {
            lexbor_free(items);
            return status;
        }
    ...
    for (size_t visited_count = 0; visited_count < item_count;
        ...
        status = layout_scene_plan_append_fragment_tree(
            plan, item->fragment, index, item->offset, item->placement,
            item->sequence, item->stacking_order, NULL);
        if (status != LXB_STATUS_OK) {
            lexbor_free(items);
            return status;
        }
                              ...
                              const layout_scene_node_t *new_node)
{
    ...
    if (old_node->parent_index == LAYOUT_SCENE_NODE_NONE
        || new_node->parent_index == LAYOUT_SCENE_NODE_NONE) {
        return old_node->parent_index == LAYOUT_SCENE_NODE_NONE
            && new_node->parent_index == LAYOUT_SCENE_NODE_NONE;
    }

    if (old_plan == NULL || new_plan == NULL
        || old_node->parent_index >= old_plan->length
        || new_node->parent_index >= new_plan->length) {
        return false;
    }
                            ...
                            const layout_scene_node_t *new_node)
{
    ...
    return old_node->child_slot == new_node->child_slot
        && layout_scene_fragment_key_equal(old_node->previous_sibling_key,
                                           new_node->previous_sibling_key)
        && old_node->sequence == new_node->sequence
        && old_node->placement == new_node->placement
        && old_node->stacking_order == new_node->stacking_order;
}
                             ...
                             const layout_scene_node_t *new_node)
{
    ...
    if (!layout_scene_point_equal(old_node->offset, new_node->offset)
        || !layout_scene_rect_equal(old_node->local_rect,
                                    new_node->local_rect)
        || old_node->stacking_order != new_node->stacking_order) {
    ...
    if (!layout_scene_transform_equal(old_node->transform,
                                      new_node->transform)) {
        dirty |= LAYOUT_SCENE_DIRTY_TRANSFORM
            | LAYOUT_SCENE_DIRTY_COMPOSITING;
    }

    if (old_node->clip_axes != new_node->clip_axes
        || !layout_scene_rect_equal(old_node->clip_rect,
                                    new_node->clip_rect)) {
        dirty |= LAYOUT_SCENE_DIRTY_CLIP | LAYOUT_SCENE_DIRTY_COMPOSITING;
    }

    if (old_node->opacity != new_node->opacity) {
        dirty |= LAYOUT_SCENE_DIRTY_OPACITY
            | LAYOUT_SCENE_DIRTY_COMPOSITING;
    }
    ...
    if (old_node->flags != new_node->flags) {
        ...
        if (((old_node->flags ^ new_node->flags)
            ...
            dirty |= LAYOUT_SCENE_DIRTY_NATIVE_EMBED
                | LAYOUT_SCENE_DIRTY_COMPOSITING;
        }
    }
...
void
layout_scene_plan_clean(layout_scene_plan_t *plan)
{
    layout_t *layout;
...
layout_scene_plan_build(layout_scene_plan_t *plan, layout_result_t *result)
{
    ...
    if (result->root_fragment == NULL || !result->frozen
        || layout_result_needs_layout(result)) {
        return LXB_STATUS_ERROR_WRONG_STAGE;
    }
    ...
    for (size_t i = 0; i < result->oofs_length; i++) {
        ...
        if (layout_result_oof_containing_fragment(result, &result->oofs[i])
            == NULL) {
            return LXB_STATUS_ERROR_NOT_EXISTS;
        }
    }
    ...
    status = layout_scene_plan_append_fragment_tree(
        plan, result->root_fragment, LAYOUT_SCENE_NODE_NONE, root_offset,
        LAYOUT_FRAGMENT_PLACEMENT_NORMAL, 0,
        layout_fragment_stacking_order(result->root_fragment), NULL);
    if (status != LXB_STATUS_OK) {
        layout_scene_plan_clean(plan);
        return status;
    }
...
int
layout_scene_node_stacking_order(const layout_scene_node_t *node)
{
...
void
layout_scene_diff_clean(layout_scene_diff_t *diff)
{
    if (diff == NULL) {
        return;
    }
                                        ...
                                        const bool *used)
{
    ...
    for (size_t i = 0; i < plan->length; i++) {
        if ((used == NULL || !used[i])
            && layout_scene_fragment_key_equal(plan->nodes[i].key, key)) {
            return i;
        }
    }
...
layout_scene_plan_find_unused_child_key_index(layout_scene_plan_t *plan,
                                             ...
                                             const bool *used)
{
    if (plan == NULL || parent == NULL || first_slot > last_slot
        || last_slot > parent->child_count) {
        return LAYOUT_SCENE_NODE_NONE;
    }
    ...
    for (size_t slot = first_slot; slot < last_slot; slot++) {
        ...
        if ((used == NULL || !used[index])
            && layout_scene_fragment_key_equal(plan->nodes[index].key, key)) {
            return index;
        }
    }
...
{
    lxb_status_t status;
    unsigned dirty = LAYOUT_SCENE_DIRTY_CHILD_LIST
        | LAYOUT_SCENE_DIRTY_GEOMETRY | LAYOUT_SCENE_DIRTY_PAINT;
    ...
    status = layout_scene_diff_append_node(diff, LAYOUT_SCENE_PATCH_INSERT,
                                           NULL, new_node, dirty);
    if (status != LXB_STATUS_OK) {
        return status;
    }
                              ...
                              bool *old_used)
{
    ...
    status = layout_scene_diff_append_node(
        diff, LAYOUT_SCENE_PATCH_REMOVE, old_node, NULL,
                              ...
                              bool *new_used)
{
    ...
    if (old_plan == NULL || new_plan == NULL || diff == NULL
        || old_index >= old_plan->length || new_index >= new_plan->length) {
        return LXB_STATUS_ERROR_WRONG_ARGS;
    }
    ...
    moved = !layout_scene_node_same_parent(old_plan, old_node,
                                           new_plan, new_node)
        || !layout_scene_node_same_slot(old_node, new_node);
    ...
    status = layout_scene_diff_append_node(diff, op, old_node, new_node,
                                           dirty);
    if (status != LXB_STATUS_OK) {
        return status;
    }
                                ...
                                bool *new_used)
{
    ...
    if (old_plan == NULL || new_plan == NULL || old_parent == NULL
        || new_parent == NULL || diff == NULL) {
        return LXB_STATUS_ERROR_OBJECT_IS_NULL;
    }
    ...
    while (old_top < old_bottom && new_top < new_bottom) {
        ...
        if (old_index == LAYOUT_SCENE_NODE_NONE
            || new_index == LAYOUT_SCENE_NODE_NONE) {
            return LXB_STATUS_ERROR_WRONG_ARGS;
        }
        ...
        status = layout_scene_diff_emit_update(
            old_plan, old_index, new_plan, new_index, diff, old_used,
            new_used);
        if (status != LXB_STATUS_OK) {
            return status;
        }
    ...
    while (old_top < old_bottom && new_top < new_bottom) {
        size_t old_index = layout_scene_plan_child_at_slot(
            old_plan, old_parent, old_bottom - 1);
        size_t new_index = layout_scene_plan_child_at_slot(
            new_plan, new_parent, new_bottom - 1);
    ...
    for (size_t slot = new_top; slot < new_bottom; slot++) {
        ...
        old_index = layout_scene_plan_find_unused_child_key_index(
            old_plan, old_parent, old_top, old_bottom,
            new_plan->nodes[new_index].key, old_used);
        if (old_index == LAYOUT_SCENE_NODE_NONE) {
            old_index = layout_scene_plan_find_unused_key_index(
                old_plan, new_plan->nodes[new_index].key, old_used);
        }
                       ...
                       layout_scene_diff_t *diff)
{
    ...
    if (old_plan != NULL && new_plan != NULL
        && old_plan->layout != new_plan->layout) {
        return LXB_STATUS_ERROR_WRONG_ARGS;
    }
    ...
    if (old_plan != NULL && new_plan == NULL) {
        for (size_t i = 0; i < old_plan->length; i++) {
            status = layout_scene_diff_emit_remove(diff, &old_plan->nodes[i],
                                                   NULL);
            if (status != LXB_STATUS_OK) {
                return status;
            }
    ...
    if (old_plan == NULL) {
        for (size_t i = 0; i < new_plan->length; i++) {
            status = layout_scene_diff_emit_insert(diff, &new_plan->nodes[i],
                                                   NULL);
            if (status != LXB_STATUS_OK) {
                return status;
            }
    ...
    for (size_t i = 0; i < new_plan->length; i++) {
        ...
        old_index = layout_scene_plan_find_unused_key_index(
            old_plan, new_node->key, old_used);
        if (old_index != LAYOUT_SCENE_NODE_NONE
            && old_plan->nodes[old_index].parent_index
                   == LAYOUT_SCENE_NODE_NONE) {
            status = layout_scene_diff_emit_update(
                old_plan, old_index, new_plan, i, diff, old_used, new_used);
        }
        else {
    ...
    for (size_t i = 0; i < new_plan->length; i++) {
        ...
        status = layout_scene_plan_diff_children(
            old_plan, old_parent, new_plan, new_parent, diff, old_used,
            new_used);
        if (status != LXB_STATUS_OK) {
            lexbor_free(new_used);
            lexbor_free(old_used);
    ...
    for (size_t i = 0; i < new_plan->length; i++) {
        ...
        old_index = layout_scene_plan_find_unused_key_index(
            old_plan, new_plan->nodes[i].key, old_used);
        if (old_index != LAYOUT_SCENE_NODE_NONE) {
            status = layout_scene_diff_emit_update(
                old_plan, old_index, new_plan, i, diff, old_used, new_used);
        }
...
int
layout_scene_patch_old_stacking_order(const layout_scene_patch_t *patch)
{
    return patch == NULL ? 0 : patch->old_stacking_order;
}

int
layout_scene_patch_new_stacking_order(const layout_scene_patch_t *patch)
{
...
layout_object_overflow_clip(layout_object_t *object)
{
    ...
    if (non_inherited->overflow_x == LXB_CSS_OVERFLOW_X_HIDDEN
        || non_inherited->overflow_x == LXB_CSS_OVERFLOW_X_CLIP
        || non_inherited->overflow_x == LXB_CSS_OVERFLOW_X_SCROLL
        || non_inherited->overflow_x == LXB_CSS_OVERFLOW_X_AUTO) {
        clip = (layout_overflow_clip_t) (clip | LAYOUT_OVERFLOW_CLIP_X);
    }

    if (non_inherited->overflow_y == LXB_CSS_OVERFLOW_Y_HIDDEN
        || non_inherited->overflow_y == LXB_CSS_OVERFLOW_Y_CLIP
        || non_inherited->overflow_y == LXB_CSS_OVERFLOW_Y_SCROLL
        || non_inherited->overflow_y == LXB_CSS_OVERFLOW_Y_AUTO) {
...
bool
layout_transform_invert(const layout_transform_t *transform,
                        layout_transform_t *out)
{
    ...
    out->e = ((transform->c * transform->f) - (transform->d * transform->e))
        / det;
    out->f = ((transform->b * transform->e) - (transform->a * transform->f))
        / det;
                           ...
                           layout_point_t point)
{
    ...
    mapped.x = (transform->a * point.x) + (transform->c * point.y)
        + transform->e;
    mapped.y = (transform->b * point.x) + (transform->d * point.y)
        + transform->f;
                                ...
                                layout_point_t *out_local)
{
    ...
    if (!layout_fragment_find_path(root_fragment, target_fragment, links,
                                   &length,
                                   sizeof(links) / sizeof(links[0]))) {
        return LXB_STATUS_ERROR_NOT_EXISTS;
    }

    if (!layout_fragment_ancestor_to_local(root_fragment, point,
                                           coordinate_flags, &point)) {
    ...
    for (size_t i = 0; i < length; i++) {
        ...
        if (!layout_fragment_ancestor_to_local(link->fragment, point,
                                               coordinate_flags,
                                               &point)) {
            return LXB_STATUS_ERROR_UNEXPECTED_DATA;
        }
                                  ...
                                  layout_fragment_t *fragment)
{
    ...
    if (result->root_fragment == NULL || !result->frozen
        || layout_result_or_root_needs_layout(result, result->root_fragment)) {
        return LXB_STATUS_ERROR_WRONG_STAGE;
    }
                                         ...
                                         layout_point_t *out_local)
{
    ...
    if (result == NULL || oof == NULL || oof->fragment == NULL
        || out_local == NULL) {
        return LXB_STATUS_ERROR_OBJECT_IS_NULL;
    }
    ...
    if (!layout_fragment_ancestor_to_local(oof->fragment, containing_local,
                                           coordinate_flags, out_local)) {
        return LXB_STATUS_ERROR_UNEXPECTED_DATA;
    }
                                     ...
                                     layout_point_t *out_local)
{
    ...
    if (oof != NULL) {
        return layout_result_root_point_to_oof_fragment(result, oof,
                                                       root_point,
                                                       coordinate_flags,
                                                       out_local);
    }
                                ...
                                layout_point_t *out_local)
{
    return layout_result_root_point_to_fragment(
        result, target_fragment, root_point,
        LAYOUT_COORDINATE_APPLY_TRANSFORMS, out_local);
}
                                    ...
                                    layout_point_t *out_root)
{
    ...
    point = layout_fragment_local_to_ancestor(target_fragment, point,
                                             coordinate_flags);
    ...
    for (size_t i = length; i != 0; i--) {
        ...
        point.x += link->offset.x;
        point.y += link->offset.y;
        point = layout_fragment_local_to_ancestor(parent, point,
                                                 coordinate_flags);
    }

    if (target_fragment == root_fragment) {
        point = layout_fragment_local_to_ancestor(root_fragment, point,
                                                 coordinate_flags);
    }
                                      ...
                                      layout_point_t *out_root)
{
    ...
    point = layout_fragment_local_to_ancestor(oof->fragment, local_point,
                                             coordinate_flags);
    ...
    containing_oof = layout_result_find_oof_for_fragment(result,
                                                        containing_fragment);
    if (containing_oof != NULL) {
        return layout_result_oof_local_point_to_root(result, containing_oof,
                                                    point, coordinate_flags,
                                                    out_root);
    }
    ...
    return layout_fragment_local_point_to_root(result->root_fragment,
                                              containing_fragment, point,
                                              coordinate_flags, out_root);
}
                                    ...
                                    layout_rect_t *out_root_rect)
{
    ...
    for (size_t i = 0; i < 4; i++) {
        if (oof != NULL) {
            status = layout_result_oof_local_point_to_root(result, oof,
                                                          points[i],
                                                          coordinate_flags,
                                                          &points[i]);
        }
                                   ...
                                   layout_fragment_t **out_fragment)
{
    ...
    status = layout_result_root_point_to_fragment(result, fragment, root_point,
                                                 coordinate_flags, out_local);
    if (status != LXB_STATUS_OK) {
        return status;
    }
